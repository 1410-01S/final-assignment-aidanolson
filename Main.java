...
public static class World{
	...
	private static void createCreature() {
		...
		double replicationChance = 50;
		double deathChance = 20;
		double location = createLocation();
		Creature creature = new Creature(name,replicationChance,deathChance,location);
		creatures.add(creature);
	}
	
	private static void createItem() {
		int hunger = (int)(Math.random()*20);
		double location = createLocation();
		int name = (int)(Math.random()*1000000);
		Item item = new Item(name,location,hunger);
		items.add(item);
	}
	
	private static double createLocation() {
		double locationx = (int)(Math.random()*10);
		double locationy = (int)(Math.random()*10);
		double location = locationx + (locationy / 100);
		return location;
	}
	...
	public static void printCreatures(ArrayList<Creature> list) {
...
public static class Creature {
	...
	private double deathChance = 0.2;
	private double reproduceChance = 0.5;
	
	
	private Creature(String name1, double deathChance1, double reproduceChance1, double location1) {
		name = name1;
		deathChance = deathChance1;
		reproduceChance = reproduceChance1;
		location = location1;
	}
	...
	public void interact() { 
		for(int i=0;i<World.items.size();i++){
			if(World.items.get(i).getLocation() == location) {
				hunger = hunger + World.items.get(i).getHunger();
				World.items.remove(World.items.indexOf(World.items.get(i)));
			}
		}
	}
	
	private void die() {
		if (Math.random()*100 + (hunger/10) <= deathChance) {
			World.creatures.remove(World.creatures.indexOf(name));
		}
		
	}
	private void reproduce() {
		if (Math.random()*100 + (hunger/10) >= reproduceChance) {
...
public static class Item{
	...
	private Item(int name1, double location1, int hunger1) {
		name = name1;
		location = location1;
		hunger = hunger1;
	}
